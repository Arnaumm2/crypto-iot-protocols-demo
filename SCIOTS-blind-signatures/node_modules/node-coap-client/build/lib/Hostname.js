...
export function getURLSafeHostname(hostname) {
    if (isIPv6(hostname))
        return `[${hostname}]`;
    return hostname;
}
export function getSocketAddressFromURLSafeHostnameWithoutLookup(hostname) {
...
export async function getSocketAddressFromURLSafeHostname(hostname) {
    // IPv4 addresses are fine
    if (isIPv4(hostname))
        return hostname;
    // IPv6 addresses are wrapped in [], which need to be removed
