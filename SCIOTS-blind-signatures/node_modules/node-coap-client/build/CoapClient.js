...
import { createDeferredPromise } from "./lib/DeferredPromise.js";
...
import { findOption, Options } from "./Option.js";
...
class PendingRequest {
    ...
    connection;
    url;
    originalMessage; // allows resending the message, includes token and message id
    partialResponse; // allows buffering for block-wise message receipt
    retransmit;
    // either (request):
    promise;
    // or (observe)
...
export class CoapClient {
    ...
    static dtlsParams = new Map();
    /** Table of all known DTLS compat options, sorted by the hostname */
    static dtlsCompat = new Map();
    /** All pending requests, sorted by the token */
    static pendingRequestsByToken = new Map();
    static pendingRequestsByMsgID = new Map();
    static pendingRequestsByUrl = new Map();
    ...
    static reset(originOrHostname) {
        ...
        if (originOrHostname != null) {
            if (typeof originOrHostname === "string") {
                ...
                predicate = (originString) => Origin.parse(originString).hostname === originOrHostname;
            }
            else {
     ...
     */
    static async request(url, method, payload, options) {
        // parse/convert url
     ...
     */
    static async ping(target, timeout = 5000) {
        // parse/convert url
        ...
        const messageId = connection.lastMsgId = incrementMessageID(connection.lastMsgId);
        const message = CoapClient.createMessage(MessageType.CON, MessageCodes.empty, messageId);
        // remember the request
        const req = new PendingRequest({
            connection,
            url: originString,
    ...
    static requestNextBlock(request) {
        ...
        let block2Opt = findOption(message.options, "Block2");
        if (block2Opt == null) {
            // respect the response options and request the next block
            const { blockNumber, blockSize } = findOption(request.partialResponse.options, "Block2");
     ...
     * @param options - Various options to control the request.
     */
    static async observe(url, method, callback, payload, options) {
        // parse/convert url
        if (typeof url === "string") {
            url = new URL(url);
        }
    ...
    static onMessage(origin, message, _rinfo) {
            ...
            if (coapMsg.token && coapMsg.token.length) {
                ...
                if (request) {
                    ...
                    if (coapMsg.isPartialMessage()) {
                        ...
                        const blockOption = findOption(coapMsg.options, "Block2"); // we know this is != null
                    ...
                    if (CoapClient.connections.has(originString)) {
                        ...
                        const RST = CoapClient.createMessage(MessageType.RST, MessageCodes.empty, coapMsg.messageId);
    ...
    payload = null) {
        return new Message(0x01, type, code, messageId, token, options, payload);
    }
    /**
    ...
    static findRequestsByOrigin(origin) {
        ...
        return [...CoapClient.pendingRequestsByMsgID.values()]
            .filter((req) => Origin.parse(req.url).toString() === originString);
    }
    /**
     ...
     */
    static async tryToConnect(target) {
        // parse/convert url
        if (typeof target === "string") {
            target = Origin.parse(target);
        }
    ...
    static async workOffPendingConnections() {
